...
DList<T>::DList() {
    size = 0;
    head = newNode(NULL, nullptr, nullptr);
    head->prev = head;
    head->next = head;
    //DListNode<T>* DList<T>::newNode(NULL, DListNode<T>* prev, DListNode<T>* next);
}
...
DListNode<T>* DList<T>::next(DListNode<T>* node) {
    if (node == nullptr || node == head->prev) {
        return nullptr;
    }
    else {
        return node->next;
...
DListNode<T>* DList<T>::prev(DListNode<T>* node) {
    if (node == nullptr || node == head->next) {
        return nullptr;
    }
